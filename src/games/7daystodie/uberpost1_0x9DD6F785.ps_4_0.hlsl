#include "./shared.h"
// bloom on sunshafts on

// ---- Created with 3Dmigoto v1.4.1 on Tue Jun 24 03:01:15 2025
Texture3D<float4> t4 : register(t4); // LUT

Texture2D<float4> t3 : register(t3); // Black Texture

Texture2D<float4> t2 : register(t2); // Bloom

Texture2D<float4> t1 : register(t1); // Red Texture
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r1.xyzw = t4.Sample(s4_s, r0.xyz).wxyz;  // lut output, srgba
  r0.x = cmp(0.5 < cb0[42].x);
  if (r0.x != 0) {
    r0.xyz = saturate(r1.yzw);
    r1.x = dot(r0.xyz, float3(0.212672904,0.715152204,0.0721750036)); // BT709 to XYZ
  } else {
    r1.x = r0.w;
  }
  o0.xyzw = r1.yzwx;
