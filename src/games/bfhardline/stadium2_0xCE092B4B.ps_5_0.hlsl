// Concrete in episode 3, caused NaNs when behind DoF

// ---- Created with 3Dmigoto v1.4.1 on Mon Aug 11 23:14:29 2025
...
{
  float2 external_UV_Scale_default : packoffset(c0);
  float2 vc_pad0 : packoffset(c0.z);
  float2 external_UV_Offset_default : packoffset(c1);
...
Texture2D<float4> texture_Diffuse3 : register(t7);
Texture2D<float4> texture_Specular : register(t8);
Texture2D<float4> texture_Normal : register(t9);
  ...
  out float4 o3 : SV_Target3)
{
  ...
  r0.xyz = texture_Normal.Sample(sampler1_s, v5.xy).xyw;
  r0.x = r0.x * r0.z;
  ...
  r1.xy = texture_Specular.Sample(sampler1_s, v5.xy).xy;
  o0.w = external_SpecularSmoothnessScale * r1.y;
  r1.x = external_SpecularScale * r1.x;
  o1.w = sqrt(r1.x);
  r1.xyz = texture_Diffuse.Sample(sampler1_s, v5.xy).xyz;
  r2.xyzw = texture_Diffuse2.Sample(sampler1_s, v5.zw).xyzw;
  r2.xyz = r2.xyz + -r1.xyz;
  r1.xyz = r2.www * r2.xyz + r1.xyz;
  r2.xy = v5.xy * external_UV_Scale_default.xy + external_UV_Offset_default.xy;
  r2.xyzw = texture_Diffuse3.Sample(sampler1_s, r2.xy).xyzw;
  r2.xyz = r2.xyz + -r1.xyz;
  r1.w = v1.x * r2.w;
  r1.xyz = r1.www * r2.xyz + r1.xyz;
  o1.xyz = sqrt(r1.xyz);
  o2.yzw = float3(0,0,0);
  r1.xy = lightMapUvTransform.yw * v3.ww;
  r1.xy = lightMapUvTransform.xz * v2.ww + r1.xy;
