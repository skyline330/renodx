...
float4 main(
  ...
  float _602 = tonemapped.x;
  float _603 = tonemapped.y;
  float _604 = tonemapped.z;
  
  /*
  float _519 = saturate(_512);
  float _520 = saturate(_513);
  float _521 = saturate(_514);
  float _549 = select((_521 <= 0.0031308000907301903f), (_521 * 12.920000076293945f), ((exp2(log2(abs(_521)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f)) * _Lut_Params.z;
  float _550 = floor(_549);
  float _556 = ((select((_520 <= 0.0031308000907301903f), (_520 * 12.920000076293945f), ((exp2(log2(abs(_520)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f)) * _Lut_Params.z) + 0.5f) * _Lut_Params.y;
  float _558 = (((select((_519 <= 0.0031308000907301903f), (_519 * 12.920000076293945f), ((exp2(log2(abs(_519)) * 0.4166666567325592f) * 1.0549999475479126f) + -0.054999999701976776f)) * _Lut_Params.z) + 0.5f) * _Lut_Params.x) + (_550 * _Lut_Params.y);
  float _559 = _549 - _550;
  float4 _561 = _InternalLut.SampleLevel(s_linear_clamp_sampler, float2((_558 + _Lut_Params.y), _556), 0.0f);
  float4 _565 = _InternalLut.SampleLevel(s_linear_clamp_sampler, float2(_558, _556), 0.0f);
  float _575 = ((_561.x - _565.x) * _559) + _565.x;
  float _576 = ((_561.y - _565.y) * _559) + _565.y;
  float _577 = ((_561.z - _565.z) * _559) + _565.z;
  float _602 = select((_575 <= 0.040449999272823334f), (_575 * 0.07739938050508499f), exp2(log2(abs((_575 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
  float _603 = select((_576 <= 0.040449999272823334f), (_576 * 0.07739938050508499f), exp2(log2(abs((_576 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
  float _604 = select((_577 <= 0.040449999272823334f), (_577 * 0.07739938050508499f), exp2(log2(abs((_577 + 0.054999999701976776f) * 0.9478673338890076f)) * 2.4000000953674316f));
  */
  if (UberPostBasePacked1.x > 0.0f) {
    float4 _619 = _Grain_Texture.Sample(s_linear_repeat_sampler, float2(((_Grain_TilingParams.x * TEXCOORD.x) + _Grain_TilingParams.z), ((_Grain_TilingParams.y * TEXCOORD.y) + _Grain_TilingParams.w)));
    float _622 = (_619.w + -0.5f) * 2.0f;
    float _626 = 1.0f - (sqrt(dot(float3(_602, _603, _604), float3(0.2126729041337967f, 0.7151522040367126f, 0.07217500358819962f))) * UberPostBasePacked1.y);
    _640 = ((((UberPostBasePacked1.x * _602) * _622) * _626) + _602);
    _641 = ((((UberPostBasePacked1.x * _603) * _622) * _626) + _603);
    _642 = ((((UberPostBasePacked1.x * _604) * _622) * _626) + _604);
  } else {
    _640 = _602;
    _641 = _603;
    _642 = _604;
